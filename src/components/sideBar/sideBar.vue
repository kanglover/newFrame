<template>
  <div>
    <el-menu default-active="0" @open="handleOpen" class="el-menu-vertical-demo" @close="handleClose" :collapse="isCollapse"
             active-text-color="#ffd04b"  text-color="#fff"  background-color="#545c64" :router=true>
      <div class="el-menu-item is-active expand">
        <!--<router-link :to="{name: 'main'}">-->
          <img class="logo" src="data:image/svg+xml;base64," alt="">
          <h1>ICDIO</h1>
        <!--</router-link>-->
      </div>
      <!--最高级菜单-->
    <el-submenu :index="''+submenuIndex" v-for="(module, submenuIndex) in modules.nodes" :key="module.id">
      <template slot="title">
        <i class="el-icon-location"></i>
        <span>{{module.name}}</span>
      </template>
      <div  v-for="(childModule, childIndex) in module.nodes" :key="childModule.id">
        <!--包含第三级菜单-->
        <template v-if="childModule.nodes && childModule.nodes.length>0">
          <el-submenu :index="submenuIndex+'_'+childIndex">
            <template slot="title">
              <span slot="title">{{childModule.name }}</span>
            </template>
            <el-menu-item :index="submenuIndex+'_'+childIndex+'_'+nodeIndex" v-for="(childNode, nodeIndex) in childModule.nodes" :key="childNode.id">{{childNode.name}}</el-menu-item>
          </el-submenu>
        </template>
        <!--二级菜单-->
        <template v-else>
          <el-menu-item @click="addTab(childModule)" :index="childModule.router?childModule.router:submenuIndex+'_'+childIndex">{{childModule.name}}</el-menu-item>
        </template>
      </div>
    </el-submenu>
    </el-menu>
  </div>
</template>
<script>
export default {
  data () {
    return {
      isCollapse: true
    }
  },
  props: ['modules'],
  methods: {
    addTab (module) {
      this.$emit('addTab', module)
    },
    handleOpen (key, keyPath) {
      console.log(key, keyPath)
    },
    handleClose (key, keyPath) {
      console.log(key, keyPath)
    },
    change () {
      this.isCollapse = !this.isCollapse
    }
  }
}
</script>
<style scoped>
  .logo {
    width: 32px;
    height: 32px;
  }

  .logo + h1 {
    display: inline-block;
    margin: 0px;
    font-size: 20px;
    font-weight: 600;
    letter-spacing: 2px;
    padding-left: 10px;
  }
  .el-menu-vertical-demo:not(.el-menu--collapse) {
    width: 200px;
    min-height: 400px;
  }
  .expand {
    position: relative;
    padding-left: 18px;
    padding-right: 0px;
  }
</style>
